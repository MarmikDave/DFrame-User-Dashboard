  ...
  return (
    <div className="w-[100vw] p-0 m-0 h-[100vh]">
        ...
        <div className='flex flex-row'>
              <p className='px-[5vw] text-[2.65vh] flex flex-row'><span>{Object.keys(eventData).map((item,i) => (<p key={i}>You have visited <span>{Math.floor((eventData[item].time_on_site)/1000)}</span>this {item.slice(0,25)} webpage</p>))}</span><span className='flex flex-col'>{Object.keys(eventData).map(() => (<span className='px-24 text-red-500 text-2xl'>X</span>))}</span></p>
        </div>
      </div>
        </div>
      </div>   
